...
void showQualitySheet(BuildContext ctx, List<QualityVariant> qs, String? selected, Future<void> Function(QualityVariant) onSelect) {
  showModalBottomSheet(
    ...
    builder: (_) => ListView(children: [for (final q in qs) ListTile(title: Text(q.label, style: TextStyle(color: selected == q.url ? AppColors.primary : Colors.white)), trailing: selected == q.url ? const Icon(Icons.check_rounded, color: AppColors.primary) : null, onTap: () { Navigator.pop(ctx); onSelect(q); })]),
  );
}
